...
#**NEED TO CHANGE AXIS TITLES, not have so many box plots**
```
...
<p>Removing outliers will allow us to better see the core of the data in the boxplot.</p>




<h2>Data munging and wrangling - Q3 </h2>
...
diamonds %>%
  group_by(cut) %>%
  select(cut, price, carat) %>%
  mutate(select_diamonds = carat > 1.5, price > 10000) %>%
  summarise(proportion = sum(select_diamonds))
```
...
<p>The distribution of median sales is different by city in terms of the min/max of values, the variation of those values, and the number of median sales > 500. Corpus Christie has few values represented.Fort Bend has a larger variance in median prices compared to El Paso. Median prices in Collin Country are generally higher than Fort Worth.  </p><p></p>

<h2>Git and Github </h2>

<i> Github Repository:</i><p></p>
